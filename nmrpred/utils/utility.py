...
def standardize_batch_2d(batch_content, return_mask=False):
    assert len(batch_content[0].shape) >= 2
    B = len(batch_content)
    X = max([item.shape[0] for item in batch_content])
    Y = max([item.shape[1] for item in batch_content])
    output_shape = (B, X, Y) + batch_content[0].shape[2:]
    result = np.zeros(output_shape, dtype=batch_content[0].dtype)
...
def gaussian(X, sigma):
    '''
    Calculate value for gaussian functions with given sigma at position X along the last axis
    '''
    return 1/(np.sqrt(2 * np.pi) * sigma) * np.exp(-np.linalg.norm(X, axis=-1) ** 2 / (2 * sigma **2 ))

def gen_voxel_density(rel_arr, atom_types, cutoff_mask, atom_channels, grid_ticks, sigma):
    """
    Generate voxel representation for the neighbor densities with each atom in the center of the voxel box

    Parameters
    ----------
    rel_arr: ndarray with shape (B, N, 3)
        relative positional array for nearby atoms with respect to each central atom. B: batch axis, 
        N: neighbor index axis, 3: relative coordinates (x,y,z)

    atom_type: ndarray with shape (B, N)
        atom types for nearby atoms of each central atom
    
    cutoff_mask: boolean array with shape (B, N)
        indicates whether each neighbor atom is within cutoff radius of the central atom

    channels: list of int
        atomic type channels for building the voxel representation

    grid_ticks: 1d array
        the ticks for a single edge of the voxel box

    sigma: float
        sigma value for creating 3d gaussians around each neighboring atom
    """
    B = rel_arr.shape[0]
    size = len(grid_ticks)
    result_arr = np.zeros((B, len(atom_channels), size, size, size))
    grid_points = np.moveaxis(np.array(np.meshgrid(grid_ticks, grid_ticks, grid_ticks, indexing="ij")), 0, -1) # s x s x s x 3
    for b in range(B):
        masked_channel_atoms = rel_arr[b][cutoff_mask[b]]
        masked_atom_types = atom_types[b][cutoff_mask[b]]
        for channel_idx, channel in enumerate(atom_channels):
            neighbor_channel_selector = masked_atom_types == channel
            rel_coords = masked_channel_atoms[neighbor_channel_selector]   # n x 3
            result_arr[b][channel_idx] = np.sum( \
                gaussian(rel_coords[:, None, None, None, :] - grid_points[None, :, :, :, :], sigma),
                axis=0)
    return result_arr


def check_hash(arr):
    sha1_value = hashlib.sha1(np.array(arr)).hexdigest()
    return sha1_value